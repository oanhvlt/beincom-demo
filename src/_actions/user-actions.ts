...
export const searchUsers = async (searchValue: string) => {
  try {
    const users = await UserModel.find({
      name: { $regex: searchValue, $options: 'i' }, //covert dbvalue and searchValue to lowercase
    });
...
export const getFollowRequestReceived = async (userId: string) => {
  try {

    const userWithFollowRequests = await UserModel.findById(userId).populate('followRequestsReceived');

    return {
      success: true,
      data: JSON.parse(JSON.stringify(userWithFollowRequests?.followRequestsReceived)),
    };
